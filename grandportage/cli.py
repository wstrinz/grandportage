...
def cmd_table(args):
    ...
    print("  scheme_scope        EMPTY needs checked reach applicable at the target")
    ...
    print("                      base field")
    print()
    print("Certificate policy ceilings (declarations grant no authority):")
    for certificate, policy in sorted(
            K.BUILTIN_CERTIFICATE_REACH_POLICY.items()):
        print("  %-28s %s" % (certificate, policy["kind"]))
    print("  Effective reach requires a current verifier receipt on the claim.")
    print("  Exact identity replay earns CHAR_0 over Q; over F_p it earns")
    print("  FIELD_SPECIFIC(F_p). ORDERED applies to ordered fields, not C or")
    print("  algebraic-closure points. NONE means no earned EMPTY authority;")
    print("  it does not prove that the mathematical claim fails elsewhere.")
    print("  Point transport separately checks field, point universe, and embedding.")
    print()
